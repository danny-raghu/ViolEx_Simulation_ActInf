...
if size(MDP,2) > 1
    ...
    for i = 1:size(MDP,2)                  % number of MDPs
        ...
        % adjust beta? For simulation of delusion treatment
        %------------------------------------------------------------------
        if isfield(OPTIONS ,'new_beta')
%             if i > OPTIONS.new_beta_trial
%                 MDP(i).beta = OPTIONS.new_beta;
%             end
            if OPTIONS.false_inf_count > OPTIONS.false_inf_max
                MDP(i).beta = OPTIONS.new_beta;
            end

            % solve this trial (for all models synchronously)
            %--------------------------------------------------------------
            [OUT(:,i),OPTIONS] = spm_MDP_VB_X_beta(MDP(:,i),OPTIONS);
